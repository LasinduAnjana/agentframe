...
class AgentConfig:
    ...
    execution, and conversation management.
...
class Agent:
        ...
        def response_generation_node(state: Dict[str, Any]) -> Dict[str, Any]:
            ...
            try:
                ...
                if error:
                    ...
                    # Successful execution - format results
    ...
    def _format_successful_response(self, intent: Intent, execution_result: ExecutionResult) -> str:
        """Format a response when execution was successful."""
        if not execution_result.results:
            return "I completed the task successfully."

        # Try to generate a natural response using the LLM
        try:
            results_summary = []
            for step_id, result in execution_result.results.items():
    ...
    def _generate_conversational_response(self, intent: Intent) -> str:
        """Generate a conversational response for non-task intents."""
        try:
            # Get recent conversation context
            recent_messages = self.chat_history.get_recent(5)
            context_messages = [msg.to_dict() for msg in recent_messages]
    ...
    def __repr__(self) -> str:
        ...
        return (f"Agent(model={self.model.__class__.__name__}, "
